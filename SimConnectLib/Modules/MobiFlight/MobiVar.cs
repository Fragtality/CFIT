...
namespace CFIT.SimConnectLib.Modules.MobiFlight
{
    public class MobiVar : SimResource<MobiModule, MobiVar, MobiVarSubscription>
    {
        ...
        public override void Register()
        {
            ...
            {
                Call(sc => sc.AddToClientDataDefinition(
                    Id,
                    Offset * Size,
                    Size,
                    0,
            ...
            if (Type.IsString())
                Call(sc => sc.RegisterStruct<SIMCONNECT_RECV_CLIENT_DATA, MobiStringValue>(Id));
            else
                Call(sc => sc.RegisterStruct<SIMCONNECT_RECV_CLIENT_DATA, MobiVarValue>(Id));

            Call(sc => sc.RequestClientData(
                ChannelID,
                Id,
                Id,
            ...
            if (Type.IsString())
                Manager.SendClientWasmCmd($"MF.SimVars.AddString.{Name}");
            else
                Manager.SendClientWasmCmd($"MF.SimVars.Add.{Name}");
        ...
        public override void Unregister(bool disconnect)
        {
            ...
            if (disconnect && Manager.ClearDataDefinition(Id))
            {
                ...
                Call(sc => sc.ClearClientDataDefinition(Id));
            }
        ...
        public override bool WriteValue(object value)
        {
            lock ( _lock)
            {
                try
                {
                    ...
                    if (writeValue != null)
                    {
                        Manager.SendClientWasmCmd($"MF.SimVars.Set.{GetWriteCode(Name, writeValue)}");
                        return true;
                    }
                    else
